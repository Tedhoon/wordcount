...
def result(request):
    if request.method == 'POST':
        ...
        sp_sen = inputvalue.split(' ')
    textcount = len(sp_sen)
    textdic={}
    for i in sp_sen:
        if i in textdic:
            textdic[i]+=1 #키:밸류
    ...
    # for badword in inputvalue:
    for i in b_li:
        if i in inputvalue:
            badword=i
            bad_word="※"+ badword + "(이)라는 비속어가 검출되었습니다"
            return render(request, 'index.html',{'bad_word':bad_word})
        
        
    
    return render(request,'result.html', {'sp_sen':sp_sen , 'textdic' : textdic.items , 'textcount':textcount })